...
**StatixAPI** имеет главный класс, через который происходит большая часть менеджмента над BukkitAPI - `StatixAPI`, и обратиться к VaultManager можно тоже через него, например:
```java
...
// Это далеко не все функции VaultPlayer, подробнее в классе: VaultPlayer
```
### 3 | Самый простой, через VaultUtil
Вам лень получать обьект VaultPlayer? Не беда! Через VaultUtil и это не обязательно:
```java
VaultUtil.addGroup(player.getName(), "default"); // Добавляем игроку группу default
...
public class TestCommand extends BaseCommand<Player> {
    ...
    public TestCommand() {
        super("example", "testing"); // агрументы команды
    }
...
```
Также в этой разработке можно использовать анотации, для задания права для выполнения комманды, или задержки
```java
@CommandPermission(pemission = "api.testing") // - Задать право для выполнения комманды, если игрок не имеет права, то мы ему выведем сообщение
```
