package ru.statix.api.bukkit.holographic.impl;
...
import ru.statix.api.bukkit.holographic.ProtocolHolographicUpdater;
import ru.statix.api.bukkit.holographic.line.*;
import java.util.*;
import java.util.function.Consumer;

/**
 * Может быть уже совсем скоро этот класс станет незаменимым (Ну когда я его сделаю)
 * тут по задумке будет возможность анимировать строки и делать многое другое..
 */
@Getter
public class MegaHolographic implements ProtocolHolographic {
    ...
    public MegaHolographic(Location location) {
    ...
    public void removeReceivers(@NonNull Player... players) {
        ...
        for (Player player : players) {
            StatixAPI.getHologramManager().getPlayerHolographics().remove(player);
        }
    }
